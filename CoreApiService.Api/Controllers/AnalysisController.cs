    ...
    public class AnalysisController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateAnalysis([FromBody] CreateAnalysisDto dto)
        {
            ...
            try 
            {
                ...
                var response = await client.PostAsync("http://vision:8000/analyze", content);
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // سحب الصورة المشفرة
                    if (aiResult.TryGetProperty("visualExplanation", out var veProperty))
                    {
                        request.VisualExplanation = veProperty.GetString();
                    }
            ...
            return Ok(new { 
                ...
                XaiResult = request.XaiResult,
                VisualExplanation = request.VisualExplanation 
            });
        }
